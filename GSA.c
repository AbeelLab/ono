...
static PyObject* get_scored_mum(GSA * self, PyObject* args, PyObject *kwds)
{
   ...
   idx_t seq1start=0, seq2start=0, seq1end=0, seq2end=0, pg1=0, pg2=0, sg1=0, sg2=0, gappenalty=0, score=0, maxscore=0;

   int leftaligned=0, rightaligned=0;
   static char *kwlist[] = {"left_aligned", "right_aligned", NULL};

   if (!PyArg_ParseTupleAndKeywords(args, kwds, "|ii", kwlist, &leftaligned, &rightaligned))
	  return NULL;
   ...
   for (i=1; i<self->n; i++) {
	  ...
	  gappenalty=abs(pg1-pg2)+abs(sg1-sg2); //TODO: for now gappenalty is -1 and match score for all nucleotides is 1, could add posibility of a scoring matrix
	  score=l-gappenalty;
   ...
   if (maxl>0){
	  return Py_BuildValue("("PYIDXTYPE","PYIDXTYPE",I,O,O,i,i)", maxaStart, maxbStart, maxl, maxv1, maxv2, maxrcmatch,maxscore);
   } else {
	  Py_INCREF(Py_None);
	  return Py_None;
   }
...
static PyObject* get_mum(GSA * self, PyObject* args, PyObject *kwds)
{
   ...
   for (i=1; i<self->n; i++) {
	  ...
	  if (l>maxl){ //is it larger than the currently found max
		 ...
		 if (aStart>0 && bStart>0){ //if not it has to be maximal
			if ((self->T[aStart-1]!=self->T[bStart-1]) || (self->T[aStart-1]=='$') || (islower(self->T[aStart-1])) ) { //maximal?
			   continue;
			}
		 }
